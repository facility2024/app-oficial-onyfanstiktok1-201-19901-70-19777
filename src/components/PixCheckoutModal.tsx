...
export default function PixCheckoutModal({
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Monta itens (produto principal + bumps selecionados) para o gateway registrar
      // a compra no servidor ANTES de retornar o PIX. Assim o webhook sempre acha.
      const items: { product_id: string; price: number; snapshot_name: string }[] = [];
      ...
      const { data, error } = await supabase.functions.invoke("neonpay-pix-gateway", {
        body: {
          ...
          client: { phone: whatsapp, email: authData?.user?.email ?? undefined },
          items,
