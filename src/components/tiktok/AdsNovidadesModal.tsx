...
export default function AdsNovidadesModal({ open, onClose }: Props) {
  ...
  const { price: fallbackPrice } = useCheckoutPrice("novidades");
  ...
  return createPortal(
    ...
    >
      <div
        className="relative min-h-full w-full max-w-5xl mx-auto bg-gradient-to-b from-cyan-950/60 via-black to-black border-x border-cyan-500/20"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          aria-label="Voltar para Home"
          className="fixed top-3 left-3 z-[10040] flex items-center gap-1.5 px-3 h-11 rounded-full bg-black/80 border border-cyan-500/50 text-white hover:bg-cyan-800/70 shadow-[0_0_20px_rgba(34,211,238,0.5)] text-sm font-semibold"
        >
        ...
        <button
          onClick={onClose}
          aria-label="Fechar"
          className="fixed top-3 right-3 z-[10040] w-11 h-11 flex items-center justify-center rounded-full bg-black/80 border border-cyan-500/50 text-white hover:bg-cyan-800/70 shadow-[0_0_20px_rgba(34,211,238,0.5)]"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="pointer-events-none absolute -top-40 -left-40 w-[400px] h-[400px] bg-cyan-700/30 rounded-full blur-[120px]" />
        <div className="pointer-events-none absolute top-1/3 -right-40 w-[400px] h-[400px] bg-blue-600/20 rounded-full blur-[120px]" />

        <div className="relative px-3 sm:px-5 py-8 pb-16">
          <header className="text-center mb-6 sm:mb-10 pt-4">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-900/40 border border-cyan-500/40 mb-3">
              <Sparkles className="w-4 h-4 text-cyan-300" />
              <span className="text-[10px] sm:text-xs uppercase tracking-widest text-cyan-300">
                Recém-chegadas
              </span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black bg-gradient-to-r from-cyan-300 via-blue-400 to-cyan-300 bg-clip-text text-transparent drop-shadow-[0_0_30px_rgba(34,211,238,0.5)]">
              NOVIDADES
            </h1>
            <p className="mt-2 text-sm sm:text-base md:text-lg text-cyan-200/80 font-medium px-4">
              As mais recentes adicionadas à plataforma
            </p>
            ...
            <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
              {pageCards.map((card) => (
                <button
                  ...
                  className="group relative w-full aspect-[3/4] rounded-2xl overflow-hidden border border-cyan-500/30 bg-gradient-to-br from-cyan-950 to-black shadow-[0_0_20px_rgba(8,145,178,0.3)] hover:shadow-[0_0_40px_rgba(34,211,238,0.6)] hover:border-cyan-400 transition-all duration-300 hover:-translate-y-1"
                >
                  <img
                    src={card.imagem_url}
                    alt={card.nome}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center flex-wrap gap-2 mt-8">
              <Button
                ...
                disabled={page === 1}
                className="border-cyan-500/40 bg-cyan-950/40 text-cyan-200 hover:bg-cyan-800/40"
              >
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  ...
                  className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg font-bold text-sm transition-all ${
                    page === i + 1
                      ? "bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-[0_0_20px_rgba(34,211,238,0.6)]"
                      : "bg-cyan-950/40 text-cyan-200 hover:bg-cyan-800/40 border border-cyan-500/30"
                  }`}
                >
                  {i + 1}
                </button>
      ...
      {selected && (
        ...
        >
          <div
            className="relative w-full max-w-md bg-gradient-to-br from-cyan-950 to-black border border-cyan-500/40 rounded-2xl p-4 sm:p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelected(null)}
              aria-label="Fechar"
              className="absolute top-2 right-2 w-9 h-9 flex items-center justify-center rounded-full bg-black/70 border border-cyan-500/40 text-white hover:bg-cyan-800/70"
            >
              <X className="w-4 h-4" />
            </button>
            <h2 className="text-xl sm:text-2xl font-black bg-gradient-to-r from-cyan-300 to-blue-400 bg-clip-text text-transparent mb-3 pr-8">
              {selected.nome}
            </h2>
            ...
            <Button
              ...
              className="w-full mt-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold py-6 text-base shadow-[0_0_30px_rgba(34,211,238,0.6)]"
            >
      ...
      <PixCheckoutModal
        ...
        productName="Assinatura Novidades 🔥"
        storageFlag="novidades_paid"
